...
from .tokkio_aiq import TokkioNvidiaAIQService
...
async def create_pipeline_task(pipeline_metadata: PipelineMetadata):
    ...
    with tracer.start_as_current_span("ace-pipeline", context=trace_context) as span:
        ...
        if config.Pipeline.llm_processor == 'NvidiaAIQService':
            llm = TokkioNvidiaAIQService(
                filler=config.Pipeline.filler,
                time_delay=config.Pipeline.time_delay,
                aiq_server_url=config.NvidiaAIQService.aiq_server_url,
                use_knowledge_base=config.NvidiaAIQService.use_knowledge_base,
                max_tokens=config.NvidiaAIQService.max_tokens
            )
        # # For Nim use:
        # stt = RivaASRService(
        #     api_key=os.getenv("NVIDIA_API_KEY"),
